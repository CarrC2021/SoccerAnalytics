...
for player_id in player_ids:
    ...
    for sector in [CM, TENSPACE, EIGHTYARD, RIGHTHALFSPACE, RIGHTWING, RIGHTHALFSPACEDEEP, LEFTWING, LEFTHALFSPACEDEEP, LEFTHALFSPACE, DEEPLEFT, DEEPRIGHT]:
        # get the passes that were completed to the sector
        sector_passes = player_passes[(player_passes['end_x'] >= sector[0] * 105/16) & (player_passes['end_x'] <= sector[1] * 105/16) 
            & (player_passes['end_y'] >= sector[2] * 68/12) & (player_passes['end_y'] <= sector[3] * 68/12)]
    ...
    with open(f"models/xT_from_sector/{player_id}_xT_from_{TENSPACE[4]}.json", 'w') as f:

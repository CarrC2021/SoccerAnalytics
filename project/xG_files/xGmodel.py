...
class xGmodel:
    def __init__(self, model_path: str) -> None:
        ...
        if self.model_path is not None:
            if "notheaders" in self.model_path:
                self.model_variables = ["Angle", "Distance", "X", "X2", "C2"]
            else:
                self.model_variables = ["Angle", "Distance", "X2", "C2"]
        self.load_model()
    ...
    def shot_vars_from_x_and_y(
        ...
        a = np.arctan(
            7.32
            * x_coordinate
            / (x_coordinate**2 + abs(y_coordinate - 68 / 2) ** 2 - (7.32 / 2) ** 2)
        )
        if a < 0:
            a = np.pi + a
        shot_data["Angle"] = a
        shot_data["Distance"] = np.sqrt(
            x_coordinate**2 + abs(y_coordinate - 68 / 2) ** 2
        )
